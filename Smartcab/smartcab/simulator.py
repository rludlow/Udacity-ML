...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=2.0, display=True, log_metrics=False, optimized=False, alpha="none", tolerance="none", ntest="none", eps_formula="none"):
        self.env = env
        ...
        if self.log_metrics:
            ...
            #ntest = str(n_test)
            #eps = eps_formula
            scenario_name = "a-" + str(alpha) + "_tol-" + str(tolerance) + "_ntest-" + str(ntest) + "_eps-" + eps_formula
            ...
            if a.learning:
                if self.optimized: # Whether the user is optimizing the parameters and decay functions
                    ...
                    self.log_filename = os.path.join("logs", "sim_improved-learning_" + time + scenario_name + ".csv")
                    self.table_filename = os.path.join("logs","sim_improved-learning_" + time + scenario_name + ".txt")
                else:
                    self.log_filename = os.path.join("logs", "sim_default-learning_" + time + scenario_name + ".csv")
                    self.table_filename = os.path.join("logs","sim_default-learning_" + time + scenario_name + ".txt")

                self.table_file = open(self.table_filename, 'wb')
            else:
                self.log_filename = os.path.join("logs", "sim_no-learning_" + time + scenario_name + ".csv")
